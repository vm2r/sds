...
THIS_SCRIPT_EXECUTION_PATH=$(dirname $0)
...
if [ -z "${SDS_VOLUMES_MAPPING:-}" ]; then
    printf_color "red" "FATAL ERROR: Env var \"SDS_VOLUMES_MAPPING\" is not set\n\n"
    printf_color "red" "Please set this variable before running this script ($0).\n\n"
    printf_color "red" "ABORTING\n\n"
    exit 3
fi
...
if ! docker inspect --type=image "${local_image_full}" > /dev/null 2>&1; then
    ...
    printf "    - Building image. Logs at: ${build_log_file}\n"
    sleep 5
...
if docker inspect --type=container ${SDS_SDS_DOCKER_NAME} > /dev/null 2>&1; then
    ...
    if [ "$STATUS" == "running" ]; then
        ...
        ${THIS_SCRIPT_EXECUTION_PATH}/sds-start.start-container.sh
    fi
else
    #
    ...
    ${THIS_SCRIPT_EXECUTION_PATH}/sds-start.run-container.sh
