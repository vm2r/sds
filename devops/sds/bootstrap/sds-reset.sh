...
## reset-sds.sh
## 
## This scripts must run on the host machine to reset the SDS installation
##
...
if [ -z "${SDS_ROOT_IN_HOST:-}" ]; then
  echo "Error: Env var \"SDS_ROOT_IN_HOST\" is not set. Please set this variable before running this script."
  exit -1
fi
...
# STOP THE CONTAINER
#
${THIS_SCRIPT_EXECUTION_PATH}/sds-stop.sh
