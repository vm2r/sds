...
## bashrc
##
## Bash initialization script for the SDS (Standardized Development Stack) Docker container.
##
## This script is executed when a non-login bash shell starts within the SDS container.
## It initializes and configures the complete development environment needed for SDS operations.
##
## Main responsibilities:
##   - Environment setup: Sets umask, PATH, and SDS-specific environment variables
##   - GCP authentication: Verifies and initializes Google Cloud credentials
##   - Git configuration: Mirrors host .gitconfig and .ssh, sets safe directories
##   - Development tools: Initializes Python (pyenv), Node (nvm), and Pants build system
##   - Networking: Configures host aliases and network settings
##   - Terminal: Sets up custom prompt with git branch info and command execution hooks
##   - CLI tools: Defines the 'sds' command wrapper with post-execution hooks
##
## The script executes a series of initialization functions at startup and then
## unsets them to keep the environment clean. It also handles cross-platform
## differences between MacOS and Windows (WSL) host systems.
##
    ...
    if [[ ! -n "${SDS_GOOGLE_ACCOUNTS:-}" ]]; then
        for account in "${SDS_GOOGLE_ACCOUNTS[@]}"; do
            ...
            if [[ "$AUTH_RESULT_CLEAN" == "" ]]; then
                ...
                printf_color "yellow" "    - FOLLOW THE INSTRUCTIONS BELOW WITH GOOGLE ACCOUNT: ${account}\n\n"

                gcloud auth login --update-adc ${account}
                if [[ $? -ne 0 ]]; then
                    printf_color "red" "\n\n    - ERROR: Could not authenticate with Google Cloud \"${account}\".\n\n"
                fi
            else
...
initialize_path(){
    ...
    if [[ -z "${SDS_SDS_ROOT_PATH_IN_CONTAINER}" ]]; then
        printf_color "red" "\n\nERROR: SDS_SDS_ROOT_PATH_IN_CONTAINER is not set.\n\n"
        exit 1
    fi

    export SDS_HELPERS_ROOT="${SDS_SDS_ROOT_PATH_IN_CONTAINER}/opt/sds"
    export PATH="${SDS_SDS_CLI_ROOT}:${SDS_HELPERS_ROOT}:${PATH}"
...
print_toast() {                                                                 
    printf "\n\n" 
    printf_color "blue" "===============================================================\n"
    printf_color "blue" "                  INITIALIZING SDS SHELL                       \n"
    printf_color "blue" "===============================================================\n"
    printf "\n"
}
